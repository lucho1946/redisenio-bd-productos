...
COLUMNAS_ERP_WO_MATERIALES = {
    "facturar_sin_existencias",
    "producto",
    "pertenece_producto",
    "consignacion",
    "ocultar_imprimir",
...
_fila_tiene_materialidad_original = base.fila_tiene_materialidad


def fila_tiene_materialidad_v121(tabla_destino: str, fila: pd.Series, campos_negocio: list[str]) -> bool:
    """
    Regla explícita v1.21.0 para PRODUCTO_ERP_WO.

    Objetivo:
    - No depender solo de la regla genérica.
    - Evitar filas ERP_WO generadas únicamente por 0, 0.00, vacío, NO o flags por defecto.
    - Conservar fila solo cuando al menos un campo ERP_WO tenga valor material activo.
    """
    tabla_norm = base.limpiar_nombre_archivo(tabla_destino)
    fila_negocio = fila[campos_negocio]

    if tabla_norm == "producto_erp_wo":
        return base.fila_tiene_campos_materiales(
            fila_negocio,
            COLUMNAS_ERP_WO_MATERIALES,
        )

    return _fila_tiene_materialidad_original(
...
base.fila_tiene_materialidad = fila_tiene_materialidad_v121
...
def main():
    parser = argparse.ArgumentParser(
        description="Normaliza productos v1.21.0: explicita materialidad de PRODUCTO_ERP_WO y conserva decisiones v1.20.0."
    )
    ...
    reporte = {
        "script": "normalizar_tabla_productos_v1_21.py",
        "version": VERSION,
        ...
        "decision_materialidad_producto_erp_wo_v121": {
            "tabla": "producto_erp_wo",
            "regla": "Generar fila solo si al menos un campo ERP_WO tiene valor material activo.",
            "campos_materiales": sorted(COLUMNAS_ERP_WO_MATERIALES),
            "valores_no_materiales": sorted(base.VALORES_NO_MATERIALES),
            "sin_reglas_por_producto_fila_valor": True
        },
        "controles": {
            ...
            "producto_erp_wo_materialidad_explicita_v121": True,
        },
    ...
    reporte_path = reportes_dir / "reporte_normalizacion_tabla_productos_v121.json"
    ...
    print("Decision v1.21.0: PRODUCTO_ERP_WO usa materialidad explicita por campos ERP_WO.")
    print("\nResumen v1.21.0:")
